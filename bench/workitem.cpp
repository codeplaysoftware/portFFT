...
//using complex_type = sycl::vec<ftype,2>;
using complex_type = std::complex<ftype>;

void init(int size, complex_type* a, complex_type* b, complex_type* c){
    for(int i=0;i<size;i++){
        c[i] = b[i] = a[i] = {static_cast<ftype>(i*0.3), static_cast<ftype>(((N-i)%11)*0.7)};
    }
}
...
void DFT_dispatcher(T priv, std::size_t size){
    switch(size){
        #define IMPL(N) case N: sycl_fft::wi_dft<N,1,1>(priv, priv); break;
        IMPL(1)
        IMPL(2)
        IMPL(3)
        IMPL(4)
...
static void BM_dft(benchmark::State& state){
    ...
    complex_type c[N*sg_size* n_sgs];
    init(N*sg_size* n_sgs,a,b,c);

    sycl::queue q;
    complex_type* a_dev = sycl::malloc_device<complex_type>(N*sg_size* n_sgs,q);
    complex_type* b_dev = sycl::malloc_device<complex_type>(N*sg_size* n_sgs,q);
    complex_type* c_dev = sycl::malloc_device<complex_type>(N*sg_size* n_sgs,q);
    q.copy(a, a_dev, N*sg_size* n_sgs);
    q.copy(b, b_dev, N*sg_size* n_sgs);
    q.copy(c, c_dev, N*sg_size* n_sgs);
    ...
    for(auto _ : state){
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for(sycl::nd_range<1>({sg_size * n_sgs}, {sg_size}),[=](sycl::nd_item<1> it, sycl::kernel_handler kh) [[intel::reqd_sub_group_size(sg_size)]] {
                ...
                for(long j=0;j<10;j++){
                    /*dft_wrapper<N>(sycl::address_space_cast<
                                                  sycl::access::address_space::private_space, 
                                                  sycl::access::decorated::yes,
                                                  complex_type>(priv));*/
                    dft_wrapper<N>(reinterpret_cast<ftype*>(priv));
                    //it.barrier();
                    //DFT_dispatcher(reinterpret_cast<ftype*>(priv), Nn);
                }
