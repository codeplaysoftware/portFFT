...
using basic_param_tuple = std::tuple<detail::layout, direction, std::size_t, std::size_t>;
using stride_param_tuple =
    std::tuple<direction, std::size_t, std::size_t, std::vector<std::size_t>, std::vector<std::size_t>>;
using distance_param_tuple = std::tuple<direction, std::size_t, std::size_t, std::size_t, std::size_t>;
using stride_distance_param_tuple = std::tuple<direction, std::size_t, std::size_t, std::vector<std::size_t>,
...
struct test_params {
  ...
  test_params(basic_param_tuple params) : test_params() {
    layout = std::get<0>(params);
    dir = std::get<1>(params);
    batch = std::get<2>(params);
    length = std::get<3>(params);
  }
  test_params(stride_param_tuple params) : test_params() {
    dir = std::get<0>(params);
    batch = std::get<1>(params);
    length = std::get<2>(params);
    fwd_strides = std::get<3>(params);
    bwd_strides = std::get<4>(params);
  ...
  test_params(distance_param_tuple params) : test_params() {
    ...
    batch = std::get<1>(params);
    length = std::get<2>(params);
    fwd_distance = std::get<3>(params);
    bwd_distance = std::get<4>(params);
  }
  test_params(stride_distance_param_tuple params) : test_params() {
    ...
    fwd_strides = std::get<3>(params);
    bwd_strides = std::get<4>(params);
    fwd_distance = std::get<5>(params);
    bwd_distance = std::get<6>(params);
  }
};
...
template <typename FType, placement Place>
...
auto init_test(const test_params& params) {
  auto desc = get_descriptor<FType, Place>(params);
...
void check_fft_usm(const test_params& params, sycl::queue& queue) {
  ...
  auto [verif_spec, desc, host_input, host_output, tolerance] = init_test<FType, Place>(params);
  ...
  queue.copy(Place == placement::OUT_OF_PLACE ? device_output : device_input, host_output.data(), host_output.size(),
             {fft_event});
  queue.wait();
...
void check_fft_buffer(const test_params& params, sycl::queue& queue) {
  ...
  verif_spec.verify_dft(committed_descriptor.get_descriptor(),
                        Place == placement::OUT_OF_PLACE ? host_output : host_input, params.dir, tolerance);
}

template <placement Place>
void check_invalid_fft(const test_params& params, sycl::queue& queue) {
  using FType = float;
  auto desc = get_descriptor<FType, Place>(params);
