...
void cuda_reference_forward_dft(std::vector<TypeIn>& in, std::vector<TypeOut>& out,
                           std::vector<int> length, size_t offset = 0) {
  ...
  for (size_t ox = 0; ox < dims[0]; ox++) {
      for(size_t oy = 0; oy <  dims[1]; oy++){
        for(size_t oz = 0; oz < dims[2]; oz++){
          ...
          if constexpr(std::is_same<TypeOut, float2>::value){
            float2 result;
            result.x = out_temp.real();
            result.y = out_temp.imag();
            out[offset + ox * dims[1] * dims[2] + oy * dims[2] + oz] = result;
          }
          else
