...
inline void wi_dft(int dft_size, const T* in, int stride_in, T* out, int stride_out, T* privateScratch);
 ...
 * @param dft_size The size of the DFT
 ...
 * @param privateScratch Scratch memory for this WI. Expects 2 * dft_size size.
 */
template <direction Dir, typename T>
__attribute__((always_inline)) inline void naive_dft(int dft_size, const T* in, int stride_in, T* out, int stride_out,
                                                     T* privateScratch) {
#pragma clang loop unroll(full)
  for (int idx_out{0}; idx_out < dft_size; ++idx_out) {
...
#pragma clang loop unroll(full)
  for (int idx_out{0}; idx_out < 2 * dft_size; idx_out += 2) {
...
constexpr TIndex wi_temps(TIndex N) {
  ...
  constexpr int MaxRecursionLevel = 10;  // 2^10 allows dft_size < 2^10 == 1024.
 ...
 */
template <direction Dir, int RecursionLevel, typename T>
__attribute__((always_inline)) inline void wi_dft(int dft_size, const T* in, int stride_in, T* out, int stride_out,
                                                  T* privateScratch) {
  ...
  if constexpr (RecursionLevel < MaxRecursionLevel) {
    ...
    } else if (f0 >= 2 && dft_size / f0 >= 2) {
      detail::cooley_tukey_dft<Dir, RecursionLevel + 1>(dft_size / f0, f0, in, stride_in, out, stride_out,
                                                        privateScratch);
    } else {
      detail::naive_dft<Dir>(dft_size, in, stride_in, out, stride_out, privateScratch);
    }
  }
}
