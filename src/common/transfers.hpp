...
__attribute__((always_inline)) inline void global2local(detail::global_data_struct global_data, const T* global,
  ...
  global_data.log_message_local(__func__, "total_num_elems", total_num_elems, "global_offset", global_offset,
  ...
  if (local_id < unaligned_elements) {  // assuming unaligned_elements <= local_size
    ...
    global_data.log_message(__func__, "first unaligned from", global_offset + local_id, "to", local_idx, "value",
  ...
  if (my_last_idx < total_num_elems) {
    ...
    global_data.log_message(__func__, "last element from", global_offset + my_last_idx, "to", local_idx, "value",
...
__attribute__((always_inline)) inline void local2global(detail::global_data_struct global_data, const T* local,
  ...
  global_data.log_message_local(__func__, "total_num_elems", total_num_elems, "local_offset", local_offset,
  ...
  if (local_id < unaligned_elements) {  // assuming unaligned_elements <= local_size
    std::size_t local_idx = detail::pad_local<Pad>(local_offset + local_id, BankLinesPerPad);
    global_data.log_message(__func__, "first unaligned from", local_idx, "to", global_offset + local_id, "value",
                            local[local_idx]);
  ...
  if (my_last_idx < total_num_elems) {
    std::size_t local_idx = detail::pad_local<Pad>(local_offset + my_last_idx, BankLinesPerPad);
    global_data.log_message(__func__, "last element from", local_idx, "to", global_offset + my_last_idx, "value",
                            local[local_idx]);
...
__attribute__((always_inline)) inline void local2private_transposed(detail::global_data_struct global_data,
  ...
  detail::unrolled_loop<0, NumElementsPerWI, 1>([&](const int i) __attribute__((always_inline)) {
    ...
    global_data.log_message("private2local_transposed", "from", local_idx + 1, "to", 2 * i + 1, "value",
                            local[local_idx + 1]);
...
__attribute__((always_inline)) inline void global2local_transposed(detail::global_data_struct global_data,
  ...
  global_data.log_message_local(__func__, "offset", offset, "num_complex", num_complex, "stride_global", stride_global,
...
__attribute__((always_inline)) inline void private2local_transposed(detail::global_data_struct global_data,
  ...
  detail::unrolled_loop<0, NumElementsPerWI, 1>([&](const int i) __attribute__((always_inline)) {
    ...
    global_data.log_message("private2local_transposed", "from", 2 * i + 1, "to", loc_base_offset + 1, "value",
                            priv[2 * i + 1]);
...
__attribute__((always_inline)) inline void private2local_2strides(detail::global_data_struct global_data, const T* priv,
  ...
  detail::unrolled_loop<0, NumElementsPerWI, 1>([&](const int i) __attribute__((always_inline)) {
    ...
    global_data.log_message("private2local_2strides", "from", 2 * i + 1, "to", loc_base_offset + 1, "value",
                            priv[2 * i + 1]);
    local[loc_base_offset] = priv[2 * i];
    local[loc_base_offset + 1] = priv[2 * i + 1];
  });
...
__attribute__((always_inline)) inline void store_transposed(detail::global_data_struct global_data, const T* priv,
  ...
  global_data.log_message_local(__func__, "local_id", local_id, "workers_in_group", workers_in_group,
...
__attribute__((always_inline)) inline void transfer_strided(detail::global_data_struct global_data, T* priv, T* loc,
  ...
  detail::unrolled_loop<0, NumComplexElements, 1>([&](const int j) __attribute__((always_inline)) {
    ...
    if constexpr (TransferDirection == detail::transfer_direction::LOCAL_TO_PRIVATE) {
      global_data.log_message("transfer_strided", "from", detail::pad_local<Pad>(base_offset, bank_lines_per_pad), "to",
                              2 * j, "value", loc[detail::pad_local<Pad>(base_offset, bank_lines_per_pad)]);
      global_data.log_message("transfer_strided", "from", detail::pad_local<Pad>(base_offset + 1, bank_lines_per_pad),
                              "to", 2 * j + 1, "value",
    ...
    if constexpr (TransferDirection == detail::transfer_direction::PRIVATE_TO_LOCAL) {
      global_data.log_message("transfer_strided", "from", 2 * j, "to",
                              detail::pad_local<Pad>(base_offset, bank_lines_per_pad), "value", priv[2 * j]);
      global_data.log_message("transfer_strided", "from", 2 * j + 1, "to",
