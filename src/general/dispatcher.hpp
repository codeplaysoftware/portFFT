...
inline void workitem_impl(T_in input, T_out output, const sycl::local_accessor<T,1>& loc, std::size_t n_transforms, 
    ...
    for(size_t i = global_id; i < roundUpToMultiple(n_transforms, subgroup_size); i+=global_size){
        ...
        global2local(input + input_distance * (i - global_id), loc, N_reals*n_working, subgroup_size, subgroup_local_id);
        ...
        sycl::group_barrier(sg);
        local2global(loc, output + output_distance * (i - global_id), N_reals*n_working, subgroup_size, subgroup_local_id);
        sycl::group_barrier(sg);
    }
}
 ...
 * @param it sycl::nd_item<1> for the kernel launch
 */
template<int factor_wi, typename T_in, typename T_out, typename T, typename T_twiddles>
inline void subgroup_impl(int factor_sg, T_in input, T_out output, const sycl::local_accessor<T,1>& loc, std::size_t n_transforms, 
                   std::size_t input_distance, std::size_t output_distance, sycl::nd_item<1> it, T_twiddles twiddles){
    ...
    std::size_t id_of_sg_in_kernel = subgroup_id + it.get_group_linear_id() * subgroup_size;
...
void dispatcher(T_in input, T_out output, const sycl::local_accessor<T,1>& loc, std::size_t fft_size,
  ...
  // TODO: should decision which implementation to use and factorization be done on host?
  if (fits_in_wi_device<T>(fft_size)){
    workitem_dispatcher(input, output, loc, fft_size, n_transforms, input_distance, output_distance, it);
  } else{
    ...
    if(fits_in_wi_device<T>(factor_wi)){
      subgroup_dispatcher(factor_wi, factor_sg, input, output, loc, n_transforms, 
                       input_distance, output_distance, it, twiddles);
    } else{
...
T* calculate_twiddles(std::size_t fft_size, sycl::queue& q, std::size_t subgroup_size){
  if (fits_in_wi<T>(fft_size)){
    ...
    if(fits_in_wi<T>(factor_wi)){
      ...
      q.wait(); // waiting once here can be better than depending on the event for all future calls to compute
      return res;
    } else{
      throw std::exception();
    }
