...
struct committed_descriptor<Scalar, Domain>::run_kernel_struct<Dir, LayoutIn, LayoutOut, SubgroupSize, TIn,
  ...
  static sycl::event execute(committed_descriptor& desc, const TIn& in, TOut& out, const TIn& in_imag, TOut& out_imag,
    ...
    IdxGlobal vec_size = 2;//storage == complex_storage::INTERLEAVED_COMPLEX ? 2 : 1;
    ...
    for (std::size_t i = 0; i < num_batches; i += max_batches_in_l2) {
      ...
      detail::dump_device(desc.queue, "after factor 0:", desc.scratch_ptr_1.get(), desc.params.number_of_transforms * dimension_data.length * 2, l2_events);
      ...
      for (std::size_t factor_num = 1; factor_num < static_cast<std::size_t>(dimension_data.num_factors); factor_num++) {
      ...
      detail::dump_device(desc.queue, "after factor:", desc.scratch_ptr_1.get(), desc.params.number_of_transforms * dimension_data.length * 2, l2_events);
      }
