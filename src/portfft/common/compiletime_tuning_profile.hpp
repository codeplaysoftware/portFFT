...
#define PORTFFT_BUILD_NVIDIA_KERNEL_PROFILE false
#define PORTFFT_BUILD_AMD_KERNEL_PROFILE false
#define PORTFFT_BUILD_INTEL_KERNEL_PROFILE true
#define PORTFFT_BUILD_CUSTOM_KERNEL_PROFILE false
...
struct kernel_spec<ct_profile::nvidia> {
  static constexpr bool ProfileEnabled = PORTFFT_BUILD_NVIDIA_KERNEL_PROFILE;
...
struct kernel_spec<ct_profile::amd> {
  static constexpr bool ProfileEnabled = PORTFFT_BUILD_AMD_KERNEL_PROFILE;
...
struct kernel_spec<ct_profile::intel_pvc> {
  static constexpr bool ProfileEnabled = PORTFFT_BUILD_INTEL_KERNEL_PROFILE;
...
struct kernel_spec<ct_profile::custom_defined> {
  static constexpr bool ProfileEnabled = PORTFFT_BUILD_CUSTOM_KERNEL_PROFILE;
#if PORTFFT_BUILD_CUSTOM_KERNEL_PROFILE
...
template <typename KernelSpecs, template <ct_profile> class CtTrait, typename RetType>
constexpr RetType apply_ct_trait(ct_profile prof, const RetType& default_return_val) noexcept {
  if (KernelSpecs::current == prof) {
    if constexpr (kernel_spec<KernelSpecs::current>::ProfileEnabled) {
      return CtTrait<KernelSpecs::current>::Value;
    } else {
      return default_return_val;
    }
  } else {
    if constexpr (!KernelSpecs::LastProfile) {
      return apply_ct_trait<typename KernelSpecs::next, CtTrait, RetType>(prof, default_return_val);
    } else {
      return default_return_val;
    }
  }
...
constexpr std::string_view profile_name(ct_profile prof) {
  constexpr std::string_view default_res = "default";
  return ct_profile_traits::apply_ct_trait<ct_kernel_specs, ct_profile_traits::get_name, std::string_view>(prof,
                                                                                                           default_res);
}
