  ...
  for (IdxGlobal iter_value = 0; iter_value < outer_batch_product; iter_value++) {
    ...
    if (level == detail::level::WORKITEM) {
      workitem_impl<Dir, SubgroupSize, LayoutIn, LayoutOut, Scalar>(
          input + outer_batch_offset, output + outer_batch_offset, nullptr, nullptr, input_loc, batch_size, scale_factor, global_data, kh,
      ...
      subgroup_impl<Dir, SubgroupSize, LayoutIn, LayoutOut, Scalar>(
          input + outer_batch_offset, output + outer_batch_offset, nullptr, nullptr, input_loc, twiddles_loc, batch_size,
