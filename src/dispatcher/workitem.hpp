...
sycl::event committed_descriptor<Scalar, Domain>::workitem_impl::run_kernel(committed_descriptor& desc, const T_in& in, T_out& out, Scalar scale_factor, const std::vector<sycl::event>& dependencies) {
  ...
  std::size_t global_size = detail::get_global_size_workitem<Scalar>(n_transforms, Subgroup_size, desc.n_compute_units);
  ...
  return desc.queue.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for<detail::workitem_kernel<Scalar, Domain, dir, mem, transpose_in, Subgroup_size>>(
          ...
          switch (fft_size) {
            ...
            case N:                                                                                                       \
              if constexpr (detail::fits_in_wi<Scalar>(N)) {                                                                           \
                detail::workitem_impl<dir, transpose_in, N, Subgroup_size>(&in_acc_or_usm[0], &out_acc_or_usm[0], &loc[0], n_transforms, it, scale_factor); \
              }                                                                                                           \
              break;
