...
PORTFFT_INLINE void workitem_impl(const T* input, T* output, T* loc, IdxGlobal n_transforms, T scaling_factor,
                                  global_data_struct global_data, const T* load_modifier_data = nullptr,
  ...
  const Idx N = global_data.kh.get_specialization_constant<detail::SpecConstFftSize>();

  global_data.log_message_global(__func__, "entered", "N", N, "n_transforms", n_transforms);
  const Idx NReals = 2 * N;
  ...
  for (IdxGlobal i = global_id; i < round_up_to_multiple(n_transforms, static_cast<IdxGlobal>(SubgroupSize));
    ...
    if (working) {
      if (LayoutIn == detail::layout::BATCH_INTERLEAVED) {
        ...
        PORTFFT_UNROLL
        for (IdxGlobal j = 0; j < static_cast<IdxGlobal>(N); j++) {
...
template <direction Dir, Idx SubgroupSize, typename TIn, typename TOut>
      ...
      cgh.parallel_for<detail::workitem_kernel<Scalar, Domain, Dir, Mem, LayoutIn, LayoutOut, SubgroupSize>>(
          ...
          [=](sycl::nd_item<1> it, sycl::kernel_handler kh) [[sycl::reqd_sub_group_size(SubgroupSize)]] {
            ...
            detail::workitem_impl<Dir, SubgroupSize, LayoutIn, LayoutOut>(
                &in_acc_or_usm[0], &out_acc_or_usm[0], loc.get_pointer(), n_transforms, scale_factor, global_data)
