...
PORTFFT_INLINE void workgroup_impl(const T* input, T* output, T* loc, T* loc_twiddles, std::size_t n_transforms,
  ...
  global2local<level::WORKGROUP, SubgroupSize>(global_data, basic_view(twiddles), loc_twiddles_view, 2 * (M + N));
  ...
  for (std::size_t offset = global_offset; offset <= max_global_offset; offset += offset_increment) {
    if constexpr (LayoutIn == detail::layout::BATCH_INTERLEAVED) {
      ...
      if (global_data.it.get_local_linear_id() / 2 < num_batches_in_local_mem) {
        ...
        global2local_transposed<level::WORKGROUP>(global_data, input, local_view, offset / FFTSize, FFTSize,
      ...
      if (global_data.it.get_local_linear_id() / 2 < num_batches_in_local_mem) {
        ...
        local_strided_2_global_strided_transposed(local_view, output, offset, 2 * max_num_batches_in_local_mem, N, M,
                                                  FFTSize, global_data);
      }
      sycl::group_barrier(global_data.it.get_group());
    } else {
      global_data.log_message_global(__func__, "loading non-transposed data from global to local memory");
      global2local<level::WORKGROUP, SubgroupSize>(global_data, basic_view(input), local_view, 2 * FFTSize, offset);
      sycl::group_barrier(global_data.it.get_group());
      wg_dft<Dir, LayoutIn, FFTSize, N, M, SubgroupSize, BankLinesPerPad>(
      ...
      local2global_transposed(global_data, N, M, M, local_view, output, offset);
      sycl::group_barrier(global_data.it.get_group());
    }
  }
