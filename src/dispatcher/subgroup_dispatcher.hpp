...
__attribute__((always_inline)) inline void subgroup_impl(const T* input, T* output, T* loc, T* loc_twiddles,
  ...
  for (std::size_t i = id_of_fft_in_kernel; i < rounded_up_n_ffts; i += n_ffts_in_kernel) {
    ...
    if constexpr (TransposeIn == detail::transpose::TRANSPOSED) {
      ...
      if (working) {
        local2private<NRealsPerWi, pad::DO_PAD, bank_groups_per_pad>(loc, priv, subgroup_local_id, NRealsPerWi,
                                                subgroup_id * n_reals_per_sg);
      }
      sycl::group_barrier(sg);
      ...
      if constexpr (FactorSG == SubgroupSize) {
        ...
        if (working) {
          store_transposed<NRealsPerWi, pad::DONT_PAD, bank_groups_per_pad>(priv, output, id_of_wi_in_fft, FactorSG,
                                                       i * n_reals_per_sg + id_of_fft_in_sg * n_reals_per_fft);
        }
      } else {
        if (working) {
          store_transposed<NRealsPerWi, pad::DO_PAD, bank_groups_per_pad>(priv, loc, id_of_wi_in_fft, FactorSG,
