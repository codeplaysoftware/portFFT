 ...
 */
template <direction dir, int factor_wi, int factor_sg, int Subgroup_size, typename T>
 ...
 * @param scaling_factor Scaling factor applied to the result
 */
template <direction dir, int factor_wi, int Subgroup_size, typename T>
...
sycl::event committed_descriptor<Scalar, Domain>::subgroup_impl::run_kernel(committed_descriptor& desc, const T_in& in, T_out& out, Scalar scale_factor, const std::vector<sycl::event>& dependencies) {
  ...
  std::size_t global_size = detail::get_global_size_subgroup<Scalar>(n_transforms, static_cast<std::size_t>(factor_sg), Subgroup_size, desc.n_compute_units);
  ...
  return desc.queue.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for<detail::subgroup_kernel<Scalar, Domain, dir, mem, transpose_in, Subgroup_size>>(
...
#define SYCL_FFT_SG_WI_DISPATCHER_IMPL(N)                                                                       \
      case N:                                                                                                       \
        if constexpr (detail::fits_in_wi<Scalar>(N)) {                                                                           \
          detail::cross_sg_dispatcher<dir, N, Subgroup_size>(factor_sg, &in_acc_or_usm[0], &out_acc_or_usm[0], &loc[0], &loc_twiddles[0], n_transforms, it, \
...
template <typename Scalar, domain Domain>
void committed_descriptor<Scalar, Domain>::subgroup_impl::set_spec_constants(committed_descriptor& desc, sycl::kernel_bundle<sycl::bundle_state::input>& in_bundle){
